...
class GlobeRenderer {
  ...
  // Compute rotationGroup.rotation.y needed to face a longitude toward the camera
  _targetRotationYForLon(lon) {
    return -(lon + 90) * Math.PI / 180;
  }

  // Smoothly rotate the globe so the given lat/lon faces the camera.
  // Returns a promise that resolves when the animation completes.
  rotateTo(lat, lon, duration = 1.5) {
    const targetY = this._targetRotationYForLon(lon);
    const currentY = this.rotationGroup.rotation.y;

    // Shortest-path difference in [-PI, PI]
    ...
    return new Promise(resolve => {
      this._rotateAnim.resolve = resolve;
    });
  }

  // Called each frame from the animation loop.
  // Returns true while animating (caller should suppress normal rotation).
  updateRotation(delta) {
    if (!this._rotateAnim) return false;
    const anim = this._rotateAnim;
    anim.elapsed += delta;
    const t = Math.min(1, anim.elapsed / anim.duration);
    // Ease in-out quadratic
    const eased = t < 0.5 ? 2 * t * t : 1 - Math.pow(-2 * t + 2, 2) / 2;
    this.rotationGroup.rotation.y = anim.startY + (anim.endY - anim.startY) * eased;
    if (t >= 1) {
  ...
  cancelRotation() {
    if (this._rotateAnim) {
      const resolve = this._rotateAnim.resolve;
      this._rotateAnim = null;
      if (resolve) resolve();
    }
  }
