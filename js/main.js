...
(async function () {
  ...
  // === Time factor arrow buttons (mouse/touch press-and-hold) ===
  const tfBtnDown = document.getElementById('tf-btn-down');
  const tfBtnUp = document.getElementById('tf-btn-up');

  function startAdjust(dir) {
    if (timeAdjustDir === dir) return;
    timeAdjustDir = dir;
    timeAdjustHeldSec = 0;
    if (!animateRunning) startEarlyAdjust();
  }

  function stopAdjust(dir) {
    if (timeAdjustDir !== dir) return;
    timeAdjustDir = 0;
    timeAdjustHeldSec = 0;
    localStorage.setItem('wopr_timeFactor', TIME_COMPRESSION);
  ...
  for (const [btn, dir] of [[tfBtnDown, -1], [tfBtnUp, 1]]) {
    btn.addEventListener('mousedown', (e) => { e.preventDefault(); startAdjust(dir); });
    btn.addEventListener('touchstart', (e) => { e.preventDefault(); startAdjust(dir); });
  }
  document.addEventListener('mouseup', () => { if (timeAdjustDir) stopAdjust(timeAdjustDir); });
  document.addEventListener('touchend', () => { if (timeAdjustDir) stopAdjust(timeAdjustDir); });
  document.addEventListener('touchcancel', () => { if (timeAdjustDir) stopAdjust(timeAdjustDir); });

  // Swap to vertical arrows on mobile
  if (document.documentElement.classList.contains('mobile-layout') ||
      window.matchMedia('(max-width: 600px)').matches) {
    tfBtnDown.textContent = '\u25BC';
    tfBtnUp.textContent = '\u25B2';
  }
  ...
  async function runScenario(scenarioName) {
    ...
    // In 3D mode, start rotating the globe to the first launch origin (non-blocking)
    let rotatePromise = null;
    if (USE_3D_GLOBE && sequence.missiles.length > 0) {
      const first = sequence.missiles[0];
      const lonOffset = 7.5 * (TIME_COMPRESSION / BASE_TIME_COMPRESSION);
      rotatePromise = globe3d.rotateTo(first.origin.lat, first.origin.lon + lonOffset, 1.5);
    }
    ...
    // Ensure globe rotation finishes before launching missiles
    if (rotatePromise) {
      await rotatePromise;
      if (aborted()) return;
    }
  ...
  function animate() {
    ...
    // If a rotateTo animation is active, let it drive rotation instead.
    if (globe3d.rotationGroup) {
      if (!globe3d.updateRotation(delta)) {
